...
export class Rule extends Lint.Rules.AbstractRule {
  ...
  public apply(sourceFile: ts.SourceFile): Lint.RuleFailure[] {
    return this.applyWithWalker(
      new ImportsContainmentWalker(sourceFile, this.getOptions())
    );
  }
}
...
class ImportsContainmentWalker extends Lint.RuleWalker {
  public visitImportDeclaration(node: ts.ImportDeclaration): void {
    ...
    if (sourceFileRelativePath) {
      ...
      const sourceDirRelativePath = sourceFileRelativePath.substring(
        0,
        sourceFileRelativePath.lastIndexOf("/")
      );
      ...
      if (sourceDirRelativePath.length > containmentPath.length) {
        ...
        if (importRelativePath) {
          ...
          const importFileName = importRelativePath.substring(
            importRelativePath.lastIndexOf("/") + 1
          );
          // Get how many levels below the containment path the file resides
          const levelsBelowPath = getLevelsBelowPath(
            containmentPath,
...
function getSourceFilePathRelativeToContainmentPath(
  ...
  if (indexOfContainmentPath !== -1) {
    const relativeSourceFilePath = sourceFilePath.substr(
      indexOfContainmentPath
    );
